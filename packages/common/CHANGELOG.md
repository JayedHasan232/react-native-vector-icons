## 12.0.0-alpha.25 (2025-03-29)

### 🩹 Fixes

- **common:** switch get-image to be an optional peer dependency ([1f4d5974](https://github.com/oblador/react-native-vector-icons/commit/1f4d5974))

### ❤️ Thank You

- John Ferlito @johnf

## 12.0.0-alpha.24 (2025-03-28)

### 🚀 Features

- fonts no longer track upstream version ([474378fb](https://github.com/oblador/react-native-vector-icons/commit/474378fb))
- **icon-explorer:** switch to react-native-owl from detox ([132bdc48](https://github.com/oblador/react-native-vector-icons/commit/132bdc48))
- aliginging with latest create-react-native-library (WIP) ([917be325](https://github.com/oblador/react-native-vector-icons/commit/917be325))
- improve new arch support ([#1652](https://github.com/oblador/react-native-vector-icons/pull/1652))
- dynamic font loading on native platforms ([#1645](https://github.com/oblador/react-native-vector-icons/pull/1645))

### 🩹 Fixes

- make the extra font warning yellow ([#1707](https://github.com/oblador/react-native-vector-icons/pull/1707))
- update typescript to check prerelease versions ([ed805e66](https://github.com/oblador/react-native-vector-icons/commit/ed805e66))
- add types for older builders ([d853cfbf](https://github.com/oblador/react-native-vector-icons/commit/d853cfbf))
- update node modules ([e5d01106](https://github.com/oblador/react-native-vector-icons/commit/e5d01106))
- move ios copy font script out of podspec and into file ([961b6f3e](https://github.com/oblador/react-native-vector-icons/commit/961b6f3e))
- don't ship the codegen files ([32ad0b17](https://github.com/oblador/react-native-vector-icons/commit/32ad0b17))
- back out autogeneration of spec files as it breaks on RN0.73 ([c31b9c7e](https://github.com/oblador/react-native-vector-icons/commit/c31b9c7e))
- common module needs to ship react-native.config.json ([bd8fb53d](https://github.com/oblador/react-native-vector-icons/commit/bd8fb53d))
- make sure rnvi-update-plist is executable ([0458e4d5](https://github.com/oblador/react-native-vector-icons/commit/0458e4d5))
- generated ([d9669835](https://github.com/oblador/react-native-vector-icons/commit/d9669835))
- andorid <0.74 ([e6936a48](https://github.com/oblador/react-native-vector-icons/commit/e6936a48))
- s this it? ([4420a2a2](https://github.com/oblador/react-native-vector-icons/commit/4420a2a2))
- don't generate the codegen for now ([05d8d219](https://github.com/oblador/react-native-vector-icons/commit/05d8d219))
- update node modules ([1951239e](https://github.com/oblador/react-native-vector-icons/commit/1951239e))
- remove example ([9af552e3](https://github.com/oblador/react-native-vector-icons/commit/9af552e3))
- align with latest create-react-native-library ([7a9fceab](https://github.com/oblador/react-native-vector-icons/commit/7a9fceab))

### 🧱 Updated Dependencies

- Updated @react-native-vector-icons/get-image to 12.0.0-alpha.24
- Updated @react-native-vector-icons/get-image to 12.0.0-alpha.24

### ❤️ Thank You

- John Ferlito @johnf
- Vojtech Novak @vonovak